...
Interrupted sessions can be resumed from their last state. zao preserves all events and works in-progress, so resume continues where you left off.

## Basic Resume

```bash
zao continue <session_id>
```
...
zao continue <session_id> --recent-events 10

# Include last 100 events
zao continue <session_id> --recent-events 100
```
...
zao branch <session_id> --from-checkpoint <checkpoint_id>

# Resume the branch
zao continue <new_branch_id>
```
